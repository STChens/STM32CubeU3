...
/**
  ******************************************************************************
  * @file    RTC_TrustZone/Secure/Src/main.c
...
/*          mapped in the non-secure vector table
                         */
#ifdef VECT_TAB_SRAM
  #define VECT_TAB_NS_OFFSET  0x10000
  #define VTOR_TABLE_NS_START_ADDR (SRAM1_BASE_NS|VECT_TAB_NS_OFFSET)
#else
  #define VECT_TAB_NS_OFFSET  0x80000
  #define VTOR_TABLE_NS_START_ADDR (FLASH_BASE_NS|VECT_TAB_NS_OFFSET)
#endif /* VECT_TAB_SRAM */
...
int main(void)
{
  ...
  /* Release I/O for LD2 control on non-secure */
...
static void MX_RTC_Init(void)
{
  ...
  */
  ...
  privilegeState.backupRegisterStartZone2 = RTC_BKP_DR10;
  privilegeState.backupRegisterStartZone3 = RTC_BKP_DR20;
  if (HAL_RTCEx_PrivilegeModeSet(&hrtc, &privilegeState) != HAL_OK)
  {
    Error_Handler();
  }
  secureState.rtcSecureFull = RTC_SECURE_FULL_NO;
  secureState.rtcNonSecureFeatures = RTC_NONSECURE_FEATURE_ALRB;
...
/**
  * @brief  All backup registers can be R/W by the secure SW.
  * @param  None
  * @retval None
  */
static void  RTC_TestBackUpRegisters(void)
{
  for (uint32_t i = 0; i < RTC_BKP_NB; i++)
  {
    /* Write backup register */
    HAL_RTCEx_BKUPWrite(&hrtc, i, i);
    /* Read backup register */
    if (HAL_RTCEx_BKUPRead(&hrtc, i) != i)
    {
      Error_Handler();
    }
  }
...
/* Limitation [EWARM-9484] with EWARM V9.20 */
...
void HAL_GTZC_TZIC_Callback(uint32_t PeriphId)
{
  ...
  if( (PeriphId == GTZC_PERIPH_RTC)|| (PeriphId == GTZC_PERIPH_TAMP))
  {
