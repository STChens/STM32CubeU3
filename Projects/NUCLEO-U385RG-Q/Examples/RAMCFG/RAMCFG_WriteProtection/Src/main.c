...
static void MX_GPDMA1_Init(void)
{
  ...
  handle_GPDMA1_Channel11.Init.Request = DMA_REQUEST_SW;
  handle_GPDMA1_Channel11.Init.BlkHWRequest = DMA_BREQ_SINGLE_BURST;
  handle_GPDMA1_Channel11.Init.Direction = DMA_MEMORY_TO_MEMORY;
  handle_GPDMA1_Channel11.Init.SrcInc = DMA_SINC_INCREMENTED;
  handle_GPDMA1_Channel11.Init.DestInc = DMA_DINC_INCREMENTED;
  handle_GPDMA1_Channel11.Init.SrcDataWidth = DMA_SRC_DATAWIDTH_WORD;
  handle_GPDMA1_Channel11.Init.DestDataWidth = DMA_DEST_DATAWIDTH_WORD;
  ...
  handle_GPDMA1_Channel11.Init.TransferEventMode = DMA_TCEM_REPEATED_BLOCK_TRANSFER;
  ...
  if (HAL_DMAEx_ConfigRepeatBlock(&handle_GPDMA1_Channel11, &RepeatBlockConfig) != HAL_OK)
  {
    Error_Handler();
  }
...
void Fill_SRAM2Page_DMA(uint32_t PageIdx)
{
  ...
  HAL_DMA_RegisterCallback(&handle_GPDMA1_Channel11, HAL_DMA_XFER_CPLT_CB_ID, TransferComplete);
  HAL_DMA_RegisterCallback(&handle_GPDMA1_Channel11, HAL_DMA_XFER_ERROR_CB_ID, TransferError);

  /* Configure the source, destination and buffer size DMA fields and Start DMA Channel/Stream transfer */
  /* Enable All the DMA interrupts */
  if (HAL_DMA_Start_IT(&handle_GPDMA1_Channel11, (uint32_t)&aSRC_Buffer, (SRAM2_BASE + (PageIdx * SRAM2_PAGE_SIZE)), (BUFFER_SIZE * 4U)) != HAL_OK)
  {
    Error_Handler();
  }
}
