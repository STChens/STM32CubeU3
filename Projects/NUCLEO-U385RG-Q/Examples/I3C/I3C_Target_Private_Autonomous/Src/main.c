...
/**
  ******************************************************************************
  * @file           : I3C/I3C_Target_Private_Autonomous/Src/main.c
  * @brief          : This sample code shows how to use STM32U3xx I3C HAL API
  *                   as Target to receive private message, using autonomous mode.
...
int main(void)
{
  ...
  if (HAL_I3C_Tgt_Receive_IT(&hi3c2, &aContextBuffers) !=  HAL_OK)
  {
    /* Error_Handler() function is called when error occurs. */
    Error_Handler();
  }
  ...
  transfer, but application may perform other tasks while transfer operation
  is ongoing. */
  while (HAL_I3C_GetState(&hi3c2) != HAL_I3C_STATE_LISTEN)
  {
  }
