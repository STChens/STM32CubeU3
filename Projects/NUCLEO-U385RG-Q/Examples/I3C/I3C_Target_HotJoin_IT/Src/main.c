...
int main(void)
{
  ...
  BSP_PB_Init(BUTTON_USER,BUTTON_MODE_GPIO);
    ...
    /* Wait for $KEY_BUTTON_NAME$ press before starting the Communication */
  while (BSP_PB_GetState(BUTTON_USER) != GPIO_PIN_RESET)
  {
  }

  /* Wait for $KEY_BUTTON_NAME$ release before starting the Communication */
  while (BSP_PB_GetState(BUTTON_USER) != GPIO_PIN_SET)
  {
  }

  /*##- Send HotJoin request ###############################################*/
...
/* USER CODE BEGIN 4 */
/**
  * @brief I3C Target Hot Join complete callback.
  *        The main objective of this user function is to check that a Hot Join process is finished
...
{
  /* Get received dynamic address */
  ubDynamicAddress = dynamicAddress;
