...
/**
  ******************************************************************************
  * @file    I3C/I3C_Controller_WakeUpFromStop/Src/main.c
  * @author  MCD Application Team
  * @brief   This sample code shows how to use STM32U3xx I3C HAL API as Controller
  *          to transmit and receive a data buffer to a Target in Stop Mode.
...
int main(void)
{
  ...
  do
  {
    ...
    /* Repeat the previous Private transfer */
    /* if a Non acknowledge is detected, due to wakeup time too long in Target side */
  }
  while (ubNackDetected == 1U);
...
static void MX_I3C1_Init(void)
{
  ...
  hi3c1.Init.CtrlBusCharacteristic.BusFreeDuration = 0x13;
...
void Error_Handler(void)
{
  ...
  /* When Acknowledge failure occurs (Target don't acknowledge it's address as it is still in wake up phase)
  Controller restarts communication */
  if (LL_I3C_IsActiveFlag_PERR(I3C1) && (LL_I3C_GetMessageErrorCode(I3C1) == LL_I3C_CONTROLLER_ERROR_CE2))
  {
    /* Clear error flag */
    LL_I3C_ClearFlag_ERR(I3C1);

    /* Inform application that a NACK have been detected, to manage reiteration of current process if needed */
    ubNackDetected = 1;
  }
  else
  {
