/**
  ******************************************************************************
  * @file    MbedTLS_HW_KWE/RSA_WrapKey_KWE/Src/main.c
 ...
 * RSSE keys.
 */
#define PSA_RSA_OAEP_KEY_ID_USER ((psa_key_id_t)0x1fff0001)
#define PSA_RSA_PSS_KEY_ID_USER  ((psa_key_id_t)0x1fff0002)
...
int main(void)
{
  ...
  psa_key_handle_t key_handle_rsa_pss;
  psa_key_handle_t key_handle_rsa_oaep;
  ...
  retval = psa_import_key(&key_attributes, rsa_key_pair, sizeof(rsa_key_pair), &key_handle_rsa_pss);
  if (retval != PSA_SUCCESS)
  {
    Error_Handler();
  }
  ...
  psa_set_key_lifetime(&key_attributes, PSA_KEY_LIFETIME_FROM_PERSISTENCE_AND_LOCATION(PSA_KEY_PERSISTENCE_DEFAULT, PSA_CRYPTO_KWE_DRIVER_LOCATION));
  /* Set up persistent Key ID */
  psa_set_key_id(&key_attributes, PSA_RSA_OAEP_KEY_ID_USER);
  /* Import the private key */
  retval = psa_import_key(&key_attributes, rsa_key_pair, sizeof(rsa_key_pair), &key_handle_rsa_oaep);
  if (retval != PSA_SUCCESS)
  {
    Error_Handler();
  }
  ...
  */
  ...
  retval = psa_destroy_key(key_handle_rsa_pss);
  if (retval != PSA_SUCCESS)
  {
    Error_Handler();
  }
  ...
  /* Destroy the private key */
  retval = psa_destroy_key(key_handle_rsa_oaep);
  if (retval != PSA_SUCCESS)
  {
    Error_Handler();
  }
