/**
  ******************************************************************************
  * @file    MbedTLS_HW_KWE/RSA_PKCS1v2.2_Sign_KWE/Src/main.c
  * @author  MCD Application Team
  * @brief   This example provides a short description of how to use the
  *          Mbed TLS PSA opaque driver with STM32 Key Wrap Engine to wrap RSA
  *          key and use the wrapped key to compute RSA PCKS1v2.2 signature.
  ******************************************************************************
  * @attention
  *
 ...
 * RSSE keys.
 */
#define PSA_RSA_PSS_KEY_ID_USER  ((psa_key_id_t)0x1fff0001)
...
uint8_t Computed_Signature_RSA[128]; /* Signature length is modulus size/8U */
...
int main(void)
{
  ...
  psa_set_key_id(&key_attributes, PSA_RSA_PSS_KEY_ID_USER);
  /* Import the private key */
  retval = psa_import_key(&key_attributes, rsa_key_pair, sizeof(rsa_key_pair), &key_handle_rsa_private);
  if (retval != PSA_SUCCESS)
  {
    Error_Handler();
  }
  ...
  psa_set_key_type(&key_attributes, PSA_KEY_TYPE_RSA_KEY_PAIR);
  psa_set_key_bits(&key_attributes, 1024);
  retval = psa_import_key(&key_attributes, rsa_key_pair, sizeof(rsa_key_pair), &key_handle_rsa_public);
  if (retval != PSA_SUCCESS)
  {
    Error_Handler();
  }
  ...
  retval = psa_sign_message(key_handle_rsa_private,
                            ...
                            Computed_Signature_RSA, sizeof(Computed_Signature_RSA), /*Data buffer to receive signature */
  ...
  if (computed_size != sizeof(Computed_Signature_RSA))
  {
    Error_Handler();
  }
  ...
  retval = psa_verify_message(key_handle_rsa_public,
                              PSA_ALG_RSA_PSS(PSA_ALG_SHA_256),                        /* Algorithm type */
                              Message, sizeof(Message),              /* Message to sign */
                              Computed_Signature_RSA, sizeof(Computed_Signature_RSA)); /* Signature to verify */
