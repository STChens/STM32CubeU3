...
IF "%oemurot_enabled%" == "0" (goto disable_wrp_oemurot)
set "command=%python%%applicfg% flash --layout %preprocess_bl2_file% -b wrp1b_start -m RE_BL2_OEMUROT_PERSO_WRP_START -d 0x1000 %ob_flash_programming% --vb >> %current_log_file% 2>>&1"
%command%
IF !errorlevel! NEQ 0 goto :error

set "command=%python%%applicfg% flash --layout %preprocess_bl2_file% -b wrp1b_end -m RE_BL2_OEMUROT_PERSO_WRP_END -d 0x1000 %ob_flash_programming% --vb >> %current_log_file% 2>>&1"
%command%
IF !errorlevel! NEQ 0 goto :error
