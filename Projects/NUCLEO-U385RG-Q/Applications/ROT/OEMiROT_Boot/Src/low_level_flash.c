...
#define SIZEOF_DOUBLE_WORD          (sizeof(uint64_t))
...
static int32_t Flash_Initialize(ARM_Flash_SignalEvent_t cb_event)
{
  ...
  err = FLASH_WaitForLastOperation(FLASH_TIMEOUT_VALUE);
...
static int32_t Flash_ReadData(uint32_t addr, void *data, uint32_t cnt)
{
  ...
  else
  {
    ...
    BOOT_LOG_ERR("Double ECC error detected: FLASH_ECCCR=0x%x", FLASH->ECCCR);
                                 ...
                                 const void *data, uint32_t cnt)
{
  ...
  */
  ...
  do
  {
    memcpy(buf, (void *)((uint32_t)data + loop), SIZEOF_DOUBLE_WORD);
    if ((buf[0] != -1) || (buf[1] != -1))
    {
    ...
    loop += SIZEOF_DOUBLE_WORD;
    addr += SIZEOF_DOUBLE_WORD;
  ...
  if (((is_range_secure(&ARM_FLASH0_DEV, address, cnt)) &&
       ((flash_base != (uint32_t)FLASH_BASE_S) || (write_type != FLASH_TYPEPROGRAM_DOUBLEWORD)))
      || ((!is_range_secure(&ARM_FLASH0_DEV, address, cnt)) &&
      ((flash_base != (uint32_t)FLASH_BASE_NS) || (write_type != FLASH_TYPEPROGRAM_DOUBLEWORD_NS))))
  {
    err = HAL_ERROR;
  }
  ...
  if (err != HAL_OK)
  {
    ...
    BOOT_LOG_ERR("->>> Failed to write 0x%" PRIx32 " bytes at 0x%" PRIx32 " (Error 0x%" PRIx32 ")", cnt,
                 (uint32_t)FLASH_BASE + addr, flashError);
  }
...
static int32_t Flash_EraseSector(uint32_t addr)
{
  ...
  {
    flashError = HAL_FLASH_GetError();
    BOOT_LOG_ERR("->>> Erase failed at 0x%" PRIx32 " (Error 0x%" PRIx32 ")", (uint32_t)FLASH_BASE + addr, flashError);
  }
