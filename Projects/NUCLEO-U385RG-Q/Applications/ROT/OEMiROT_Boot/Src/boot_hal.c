...
static uint32_t GetPage(uint32_t Addr)
{
  ...
  {
    /* Bank 2 */
    page = (Addr - (FLASH_BASE + FLASH_B_SIZE)) / FLASH_PAGE_SIZE;
  }
...
void boot_platform_quit(struct boot_arm_vector_table *vector)
{
  ...
  flash_hdpext.Bank    = GetBank(FLASH_BASE + FLASH_HASH_REF_AREA_OFFSET);
  flash_hdpext.NbPages = GetPage(FLASH_BASE + FLASH_BL2_HDP_END_OFFSET) - GetPage(FLASH_BASE + FLASH_HASH_REF_AREA_OFFSET);
  HAL_FLASHEx_ConfigHDPExtension(&flash_hdpext);

  boot_jump_to_RSSExt((uint32_t)&boot_jump_to_RSSExt, rsslib_sec_Close_Exit_HDPExt, RSSLIB_HDP_AREA1_Msk, (uint32_t) vt);
#else  /* OEMUROT_ENABLE */
  ...
  boot_jump_to_RSS((uint32_t)&boot_jump_to_RSS, rsslib_sec_Close_Exit_HDP, RSSLIB_HDP_AREA1_Msk, (uint32_t) vt);
#endif  /* OEMUROT_ENABLE */
...
/**
  ...
  *         AHB/APBx prescalers and Flash settings for System clock 96MHz
  * @retval None
  */
static void SetSysClock(void)
{
