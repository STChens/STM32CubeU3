...
TX_THREAD             cdc_acm_receive_thread;
...
UINT MX_USBX_Host_Init(VOID *memory_ptr)
{
  ...
  /* Create the cdc_acm_receive thread */
  if (tx_thread_create(&cdc_acm_receive_thread, "CDC Receive App thread",
                       cdc_acm_receive_app_thread_entry, 0, pointer,
...
VOID USBX_APP_Host_Init(VOID)
{
  ...
  USBH_UsrLog("Starting CDC ACM Application");
  USBH_UsrLog("Connect your CDC ACM Device");
