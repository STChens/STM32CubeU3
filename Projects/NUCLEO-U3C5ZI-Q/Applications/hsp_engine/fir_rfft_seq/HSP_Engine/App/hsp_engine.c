...
void MX_HSP_Engine_Init(void)
{
  ...
  /* USER CODE BEGIN HSP_Engine_Init_PostBRAMAlloc */

  /* USER CODE END HSP_Engine_Init_PostBRAMAlloc */

  /* USER CODE BEGIN HSP_Engine_Init_PostPLRecord */
  MX_HSP_SEQ_Record_PL_FIR_FFT(&hmw);
  /* USER CODE END HSP_Engine_Init_PostPLRecord */

  /* Configure the Trigger Output */
  if (HSP_CORE_OUTPUT_SetConfig(&hmw, HSP_CORE_TRGO_0, HSP_CORE_TRGO_SRC_SPE) != HSP_CORE_OK)
  {
    Error_Handler();
  }
  ...
  if (HSP_SEQ_EVENT_Enable(&hmw, HSP_SEQ_ALL_EVENTS_MASK) != HSP_CORE_OK)
  {
    Error_Handler();
  }
