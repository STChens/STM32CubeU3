...
uint32_t MX_HSP_BRAM_Allocation(hsp_engine_context_t *hmw)
{
  ...
  g_bram_resources.p_buff_rfft = (float32_t *)HSP_BRAM_Malloc(hmw, HSP_BRAM_RSRC_INPUT_SAMPLES_SIZE, HSP_BRAM_ALLOCATION_DEFAULT);
  if (g_bram_resources.p_buff_rfft == NULL) error++;

  g_bram_resources.p_rfft_magnitude = (float32_t *)HSP_BRAM_Malloc(hmw, HSP_BRAM_RSRC_RFFT_RESULT_SIZE, HSP_BRAM_ALLOCATION_DEFAULT);
  if (g_bram_resources.p_rfft_magnitude == NULL) error++;
