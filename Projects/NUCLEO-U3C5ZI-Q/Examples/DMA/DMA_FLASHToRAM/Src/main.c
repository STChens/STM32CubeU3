...
/**
  ...
  *          SRAM memory through the STM32U3xx HAL API.
  ******************************************************************************
  * @attention
  *
...
/* USER CODE BEGIN PFP */
/* Private function prototypes -----------------------------------------------*/
static void TransferComplete(DMA_HandleTypeDef *handle_GPDMA1_Channel1);
static void TransferError(DMA_HandleTypeDef *handle_GPDMA1_Channel1);
...
int main(void)
{
  ...
  HAL_DMA_RegisterCallback(&handle_GPDMA1_Channel1, HAL_DMA_XFER_CPLT_CB_ID, TransferComplete);
  ...
  if (HAL_DMA_Start_IT(&handle_GPDMA1_Channel1, (uint32_t)&aSRC_Const_Buffer, (uint32_t)&aDST_Buffer, sizeof(aDST_Buffer)) != HAL_OK)
  {
    /* Transfer Error */
    Error_Handler();
  }
