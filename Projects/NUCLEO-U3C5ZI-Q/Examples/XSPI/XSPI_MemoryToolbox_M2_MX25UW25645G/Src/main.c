...
/* USER CODE BEGIN PFP */
static Memory_Toolbox_Status_t M2_MX25UW25645G_NorFlash_Configuration(XSPI_HandleTypeDef *hxspi);
...
int main(void)
{
  ...
  */
  ...
  /* Program Step 4 : Configure the Nor Flash memory (Drive strength, Dummy Cycles, Octal DTR mode) */
  EXECUTE_AND_CHECK(M2_MX25UW25645G_NorFlash_Configuration(&hxspi1));
...
/**
  * @brief  Configures the memory Macronix MX25UW25645G in Octal DTR mode + Drive Strength and Dummy cycles updates.
...
static Memory_Toolbox_Status_t M2_MX25UW25645G_NorFlash_Configuration(XSPI_HandleTypeDef *hxspi)
{
  ...
  /* Print current frequency of Clock signal provided to memory in log.
     Default value is expected to be 48Mhz for configuration step */
  Memory_Toolbox_LogFrequency(&hxspi1);
#endif /* EXAMPLE_TRACE_ENABLE */

  /* Read Status Register  */
  Memory_Toolbox_ReadStatusRegister(hxspi, RegBufferRead, 1U, MEMORY_STARTUP_MODE);
  RegBufferWrite[0] = RegBufferRead[0];

  /* Read Configuration Register  */
  Memory_Toolbox_ReadConfigRegister(hxspi, RegBufferRead, 1U);

  /* Write Configuration Register to update Drive strength  */
  RegBufferWrite[1] = (RegBufferRead[0] & ~M2_MX25UW25645G_REG_CR_DRIVE_STRENGH_MASK)
                      | M2_MX25UW25645G_REG_CR_DRIVE_STRENGH_VALUE ;
  Memory_Toolbox_WriteConfigRegister(hxspi, RegBufferWrite, 2U);

  /* Write Configuration Register 2 to update Dummy Cycles */
  RegBufferWrite[0] = M2_MX25UW25645G_REG_CR2_DUMMY_CYCLE_VALUE;
  Memory_Toolbox_WriteConfigRegister2(hxspi, RegBufferWrite, 1U, M2_MX25UW25645G_REG_CR2_DUMMY_CYCLE_ADDRESS);
  ...
  Memory_Toolbox_ReadConfigRegister2(hxspi, RegBufferRead, 1U, M2_MX25UW25645G_REG_CR2_HIGH_PERF_ADDRESS);

  /* Write Configuration Register 2 to switch in Octal mode (set DOPI bit)*/
  RegBufferWrite[0] = (RegBufferRead[0] & ~M2_MX25UW25645G_REG_CR2_HIGH_PERF_MASK)
                      | M2_MX25UW25645G_REG_CR2_HIGH_PERF_VALUE ;
  Memory_Toolbox_WriteConfigRegister2(hxspi, RegBufferWrite, 1U, M2_MX25UW25645G_REG_CR2_HIGH_PERF_ADDRESS);

  /* Ensure Write operation is completed : use High Performance mode for this check, as Octal DTR is enabled */
  ...
  /* Read status Register to validate configuration */
  return (Memory_Toolbox_ReadStatusRegister(hxspi, RegBufferRead, 2U, MEMORY_HIGH_PERFORMANCE));
}
...
static void TraceOutput(const uint8_t *Msg)
{
  ...
  HAL_UART_Transmit(&huart1, (const uint8_t *)Msg, strlen((const char *)Msg), 1000U);
