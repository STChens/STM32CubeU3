...
System_StatusTypeDef system_ram_retention_config(System_LowPowerModeScenarioTypeDef scenario)
{
  ...
  __HAL_RCC_SRAM3_CLK_DISABLE();
  __HAL_RCC_SRAM4_CLK_DISABLE();
  HAL_PWREx_EnableRAMsPowerDown(PWR_SRAM1_POWERDOWN);
  HAL_PWREx_EnableRAMsPowerDown(PWR_SRAM3_POWERDOWN);
  HAL_PWREx_EnableRAMsPowerDown(PWR_SRAM4_POWERDOWN);
  }
