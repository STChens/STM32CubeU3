...
set connect_hardware_reset=-c port=SWD mode=powerdown -hardRst
...
echo %action%
echo "Configure Secure option Bytes: Write Protection, Hide Protection"
%stm32programmercli% %connect% -ob %boot_write_protect% %hide_protect_1% %wrp_protect%
IF !errorlevel! NEQ 0 goto :error

:retry_sec_water_mark
set "action=Do HW reset"
echo %action%
%stm32programmercli% %connect_hardware_reset%
IF !errorlevel! NEQ 0 goto :error

set "action=Set sec watermark"
echo %action%
echo "Configure Secure option Bytes: sec watermark"
%stm32programmercli% %connect_no_reset% -ob %sec_water_mark%
IF !errorlevel! NEQ 0 (
echo Waiting for 5 seconds...
timeout /t 5 /nobreak >nul

goto :retry_sec_water_mark
)

set "action=Configure Secure option Bytes: boot lock"
echo %action%
echo "Configure Secure option Bytes: Write Protection, Hide Protection and boot lock"
%stm32programmercli% %connect% -ob %boot_lock%
IF !errorlevel! NEQ 0 goto :error
